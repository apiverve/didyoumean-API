...
class DidyoumeanValidationException implements Exception {
  final List<String> errors;

  DidyoumeanValidationException(this.errors);

  @override
  String toString() => 'DidyoumeanValidationException: ${errors.join("; ")}';
}
...
/// Did You Mean API client.
///
/// For more information, visit: https://apiverve.com/marketplace/didyoumean?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [query] (required) - The text you want to check for spelling mistakes
class DidyoumeanClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'query': ValidationRule(type: 'string', required: true),
  };

  DidyoumeanClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/didyoumean',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DidyoumeanValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DidyoumeanValidationException(errors);
    }
  }

  /// Execute a request to the Did You Mean API.
  ///
  /// Parameters are validated before sending the request.
  Future<DidyoumeanResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DidyoumeanException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DidyoumeanResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DidyoumeanException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DidyoumeanException('Resource not found');
      } else {
        throw DidyoumeanException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DidyoumeanException) rethrow;
      throw DidyoumeanException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Did You Mean API client.
class DidyoumeanException implements Exception {
  final String message;

  DidyoumeanException(this.message);

  @override
  String toString() => 'DidyoumeanException: $message';
}
