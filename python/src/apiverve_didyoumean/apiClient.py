...
class DidyoumeanAPIClientError(Exception):
...
class ValidationError(DidyoumeanAPIClientError):
...
class DidyoumeanAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"query": {"type": "string", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the DidyoumeanAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises DidyoumeanAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/didyoumean'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise DidyoumeanAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[DidyoumeanAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns DidYouMeanData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises DidyoumeanAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise DidyoumeanAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise DidyoumeanAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise DidyoumeanAPIClientError(f"Request failed: {str(e)}")
